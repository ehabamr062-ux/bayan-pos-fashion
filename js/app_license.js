...
window.LicenseService = {
    ...
    verifyLicense: async function () {
        ...
        if (licenseInfo.signature !== expectedSig || licenseInfo.machineId !== machineId) {
            // فحص هل الجهاز مؤهل للتجربة المجانية (أقل من 200 فاتورة) أو جهاز فرعي مقترن بالسيرفر
            const trialInvoices = typeof getEffectiveTrialInvoicesCount === 'function' ? getEffectiveTrialInvoicesCount() : 0;
            const isTrialEligible = trialInvoices < 200;
            const isPairedTerminal = (typeof getStore === 'function' && getStore('bayan_client_is_paired') === 'true');

            // إذا كان الترخيص غير متطابق ولكنه الباقة المجانية أو محظور بسبب انتقال كود الجهاز أو الجهاز ما زال في فترة التجربة أو جهاز فرعي مقترن
            if (licenseInfo.plan === 'باقة نسخة المجانية' || !licenseInfo.plan || licenseInfo.plan === 'Blocked' || isTrialEligible || isPairedTerminal) {
                console.log("🛠️ Auto-repairing license for current machine ID...");
                const freePlan = 'باقة نسخة المجانية';
                const freeExpiry = '';
                const sig = await this.generateLicenseSignature(freePlan, freeExpiry, machineId);
